...
public class Store {
    ...
    public void shop(String customer) {
        ...
        while (true) {
            ...
            if(product.equals("milk") && (warehouse.stock(product)>0 )){
                warehouse.take(product);
                cart.add(product, warehouse.price(product));
                
            }
            if(product.equals("bread") && (warehouse.stock(product)>0 )){
                warehouse.take(product);
                cart.add(product, warehouse.price(product));
                
            }
            if(product.equals("coffee") && (warehouse.stock(product)>0 )){
                warehouse.take(product);
                cart.add(product, warehouse.price(product));
                
            }
            if(product.equals("cream") && (warehouse.stock(product)>0 )){
                warehouse.take(product);
                cart.add(product, warehouse.price(product));
